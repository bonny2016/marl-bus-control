        ...
        if read == 0:
            ...
            while t < 24:
                ...
                for dest_id in stop_list.keys():
                    ...
                    mask = demand['ALIGHTING_STOP_STN'] == int(dest_id)
                    od = d.loc[mask]
                    if od.empty:
                        continue
...
def train_result_track(eng, ep, qloss_log, ploss_log, log, name='', seed=0):
    ...
    if ep % 1 == 0:
        ...
        # Collect columns for mean and variance into separate dictionaries
        mean_columns = {'headway_mean' + str(k): [np.mean(v)] for k, v in headways_mean.items()}
        var_columns = {'headway_var' + str(k): [np.mean(v)] for k, v in headways_var.items()}

        # Combine the dictionaries
        all_columns = {**mean_columns, **var_columns}

        # Add all new columns to the DataFrame at once
        train_log = pd.concat([train_log, pd.DataFrame(all_columns)], axis=1)
        ...
        try:
            ...
            print(e)
        return np.mean(reward_bus_wise)
